...
import dg.chapter3.path_filter.DateRangePathFilter;
import dg.chapter3.path_filter.RegexExcludePathFilter;
import dg.chapter3.path_filter.RegexPathFilter;
...
public class FileSystemGlobTest {
    ...
    private Set<Path> glob(String pattern) throws IOException {
        return new HashSet<>(Arrays.asList(FileUtil.stat2Paths(
                fs.globStatus(new Path(BASE_PATH + pattern)))));
    }
